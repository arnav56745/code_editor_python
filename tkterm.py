    ...
    def write(self, text):
        ...
        # Normal output
            ...
            if text.startswith(self.app.basename):
                start_pos = get_last_line(self.TerminalScreen)
                end_pos = (
                    str(start_pos).split(".")[0] + "." + str(len(self.app.basename))
                )
...
class App(tk.Frame):
    def __init__(self, parent, **kwargs):
        ...
        ## Terminal screen
        ########################################################################
        self.frameTerminal = tk.Frame(
            self, borderwidth=0, relief=FLAT, bg=self.TerminalColors["bg"]
        )
        ...
        self.frameScrollbar = Frame(
            self.frameTerminal, borderwidth=0, width=14, bg=self.TerminalColors["bg"]
        )
        ...
        self.shellMapping = {"csh": "/bin/csh", "bash": "/bin/sh", "windows": None}
        ...
        self.shellComboBox = ttk.Combobox(
            self.frameStatusBar, state="readonly", width=8, font=("Helvetica", 8)
        )
    ...
    def on_scrollbar_enter(self, event):
        ...
        self.style.configure("TScrollbar", width=10, arrowsize=10)
...
class RightClickMenu:
    def MenuBind(self):
        ...
        self.m.add_checkbutton(
            label="Show Menubar",
            variable=self.show_menu,
            onvalue=True,
            offvalue=False,
            command=self.print_values,
        )
        self.m.add_checkbutton(
            label="Fullscreen", variable=self.fullscreen, onvalue=True, offvalue=False
        )
        self.m.add_checkbutton(
            label="Read-Only", variable=self.readonly, onvalue=True, offvalue=False
        )
        self.m.add_separator()
...
class Terminal(RightClickMenu, SearchFunctionality, App):
    ...
    def __init__(self, parent, *args, **kwargs):
        ...
        self.TerminalColors = {"fg": "#E6E6E6", "bg": "#282C34"}
        ...
        if "JetBrains Mono" in font.families():
            self.terminalFont = Font(family="JetBrains Mono", size=9)
        else:
        ...
        self.TerminalScreen.tag_config("basename", foreground="#11f771", font=boldFont)
        self.TerminalScreen.tag_config("error", foreground="#f71511")
